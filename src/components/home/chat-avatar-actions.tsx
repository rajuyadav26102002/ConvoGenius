import { IMessage, useConversationStore } from "@/zustand-store/chat-store";
...
const ChatAvatarActions = ({ me, message }: ChatAvatarActionsProps) => {
	...
	return (
		...
		>
			...
			{!isMember && !fromAI && isGroup && <Ban size={18} className='text-red-500' />}
			{isGroup && isMember && selectedConversation?.admin === me._id && (
				<LogOut size={18} className='text-red-500 opacity-0 group-hover:opacity-100' onClick={handleKickUser} />
			)}
		</div>
	);
};
